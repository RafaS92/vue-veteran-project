...
describe('DashboardView', () => {
  ...
  it('renders a populated dashboard', () => {
    ...
    const store = useVeteranStore()
    store.hasLoaded = true
    store.veteran = { id: 'v1', name: 'Jordan Brooks', email: 'j@example.com', phone: '', branch: 'U.S. Navy', servicePeriod: '2012–2020', dischargeStatus: 'Honorable', location: 'Austin', preferredContact: 'Email' }
    store.benefits = [{ id: 'b1', title: 'Education Support', category: 'Education', summary: '', description: '', status: 'applied', progress: 50, nextStep: 'Upload form', saved: true }]
    store.actionItems = [{ id: 'a1', title: 'Upload form', benefitId: 'b1', dueDate: '2026-09-20', completed: false }]
    const wrapper = mount(DashboardView, { global: { plugins: [pinia], stubs: { RouterLink: { template: '<a><slot /></a>' } } } })

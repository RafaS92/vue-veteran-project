...
export function useBenefits(benefits: Ref<Benefit[]>) {
  ...
  const categories = computed(() => ['All', ...new Set(benefits.value.map((item) => item.category))])
  const filteredBenefits = computed(() => {
    const term = search.value.trim().toLowerCase()
    return benefits.value.filter((benefit) => {
      const matchesSearch = !term || `${benefit.title} ${benefit.summary}`.toLowerCase().includes(term)

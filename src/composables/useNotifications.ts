...
export interface Notification { message: string; tone: 'success' | 'info' }
interface NotificationService { notification: Readonly<Ref<Notification | null>>; notify: (message: string, tone?: Notification['tone']) => void }
...
export function provideNotifications() {
  ...
  function notify(message: string, tone: Notification['tone'] = 'success') {
    ...
    clearTimeout(timer)
    timer = setTimeout(() => { notification.value = null }, 2800)
  }
